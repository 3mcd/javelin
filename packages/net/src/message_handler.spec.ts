...
describe("createMessageHandler", () => {
  it("applies reliable component updates", () => {
    ...
    const opsA = protocol.ops([
      [WorldOpType.Spawn, 0, [{ type: 0, state: 0, x: 0, y: 0, z: 0 }]],
    ])
    const opsB = protocol.ops([
      [WorldOpType.Spawn, 1, [{ type: 0, state: 0, x: 0, y: 0, z: 0 }]],
    ])
  ...
  it("applies unreliable component updates", () => {
    ...
    const update: UpdateUnreliable = [
      ...
      // entity 0 (registered)
      0,
      { type: 0, state: 0, x: 1, y: 1 },
      // entity 1 (registered)
      1,
      { type: 0, state: 0, x: 2, y: 2 },
    ]
    const opsA = protocol.ops([
      [WorldOpType.Spawn, 0, [{ type: 0, state: 0, x: 0, y: 0 }]],
    ])
    const opsB = protocol.ops([
      [WorldOpType.Spawn, 1, [{ type: 0, state: 0, x: 0, y: 0 }]],
    ])
    ...
    expect(upsertCallArgs[0]).toEqual([0, [{ type: 0, state: 0, x: 1, y: 1 }]])
    expect(upsertCallArgs[1]).toEqual([1, [{ type: 0, state: 0, x: 2, y: 2 }]])
  })
})
