...
export const Component: React.FC<React.PropsWithChildren<ComponentProps>> = ({ children, type, values }) => {
  ...
  if (children) {
    ...
    const mergedRefs = mergeRefs([(child as any).ref, (value) => {
...
export function useComponent<T extends JComponent[]>(type: Type<T>, ...values: ComponentInitValues<T>) {
  const entity = useCurrentEntity()
