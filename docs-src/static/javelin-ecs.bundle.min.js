(function (global, factory) {
    ...
    (function (SchemaKeyKind) {
        ...
        SchemaKeyKind[SchemaKeyKind["Dynamic"] = 3] = "Dynamic";
    ...
    const isDynamicType = (object) => "__kind__" in object &&
        object.__kind__ === SchemaKeyKind.Dynamic;
    var ModelNodeKind;
    (function (ModelNodeKind) {
        ...
        ModelNodeKind[ModelNodeKind["Dynamic"] = 4] = "Dynamic";
    ...
    function registerComponentType(componentType, componentTypeId, poolSize = 1000) {
        let type = Reflect.get(componentType, $componentType);
        if (type !== undefined) {
            return;
        }
    ...
    const useInterval = createEffect(() => function useInterval(t) {
     ...
     * )
     */
    const useMonitor = createEffect(world => {
    ...
    const createRef = (initializer, options = {}) => createEffect(world => {
    ...
    const matches = (type, filters, archetype) => typeIsSuperset(archetype.signature, type) &&
    ...
    function createQueryInternal(options) {
        ...
        const layout = ((_b = options.include) !== null && _b !== void 0 ? _b : options.select).map((schema) => schema[$componentType]);
        const recordsIndex = [];
        ...
        const forEach = (iteratee) => {
            const c = context !== null && context !== void 0 ? context : UNSAFE_internals.__CURRENT_WORLD__;
        ...
        const query = forEach;
        query.not = (...exclude) => createQueryInternal({
            ...options,
            filters: {
                not: new Set(exclude.map(schema => schema[$componentType])),
            },
        });
        query.select = (...include) => createQueryInternal({
            ...options,
            include,
        });
        query.get = (entity, out = []) => {
            const c = context !== null && context !== void 0 ? context : UNSAFE_internals.__CURRENT_WORLD__;
        ...
        query.bind = (world) => createQueryInternal({
            ...options,
            context: world.id,
        });
        query.test = (entity) => {
        ...
        query.matchesArchetype = (archetype) => matches(signature, filters, archetype);
     ...
     * })
     */
    const createQuery = (...selector) => createQueryInternal({
        select: selector,
    });
    ...
    function createStorage(options = {}) {
        ...
        function relocate(source, entity, components, changed) {
